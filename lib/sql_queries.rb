...
def selects_the_titles_of_all_projects_and_their_pledge_amounts_alphabetized_by_title
  #"SELECT projects.title, SUM(pledges.amount) FROM projects, pledges ORDER BY title ASC;"#JOIN cats ON cats_owners.cat_id = cats.id JOIN user ON pledges.user_id = user.id
...
def selects_the_category_name_and_the_sum_total_of_the_all_its_pledges_for_the_books_category
  "SELECT projects.category, projects.name, pledges.amount FROM projects
    INNER JOIN pledges
    ON projects.id = pledges.project_id
    WHERE projects.category = 'music';"
end
